# -*- coding: utf-8 -*-
"""
Created on Sat Feb 22 13:18:47 2020
...
def computeDelta(wt, X, Xi):
    ...
    def similarity(a,b):
        ...
        meanA = np.mean(a)
        meanB = np.mean(b)
        res = np.dot(a-meanA,b-meanB) / (M * np.std(a) * np.std(b))
        return res 
    ...
    for i in range(len(Xi)):
      xiyi = Xi.iloc[i] 
      similarityAB = np.exp(wt * similarity(a,xiyi[:-1]))
      numeratorSum = numeratorSum + xiyi[-1] * similarityAB
      denominatorSum = denominatorSum + similarityAB
      res = numeratorSum/denominatorSum
    return res
